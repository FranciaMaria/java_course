...
public class FrmDrawing {
    ...
    private void editButtonActionPerformed() {
        ...
        if (selection instanceof Circle) {
            ...
            dlg.pack();
            dlg.setVisible(true);
        } else if (selection instanceof Donut) {
            DlgDonut dlg = new DlgDonut();
            dlg.setDonut((Donut) selection);
            dlg.pack();
            dlg.setVisible(true);
        } else if (selection instanceof Rectangle) {
            DlgRectangle dlg = new DlgRectangle();
            dlg.setRectangle((Rectangle) selection);
            dlg.pack();
            dlg.setVisible(true);
        } else if (selection instanceof Point) {
            DlgPoint dlg = new DlgPoint();
            dlg.setPoint((Point) selection);
            dlg.pack();
            dlg.setVisible(true);
        } else if (selection instanceof Line) {
            DlgLine dlg = new DlgLine();
            dlg.setLine((Line) selection);
            dlg.pack();
            dlg.setVisible(true);
        }
    ...
    private void deleteButtonActionPerformed() {
        int confirm = JOptionPane.showConfirmDialog(this.drawingPanel1, "Are you sure... ?", "Deletion", JOptionPane.YES_NO_OPTION);
